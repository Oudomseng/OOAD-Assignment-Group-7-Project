...
Module funtion
    ...
    Public Sub formulaofpayments()
        Try
            ...
            Dim ts As TimeSpan = TimeSpan.Parse(frmOverdue.txtOverdueTime.Text)
            'Dim TValueMin As Integer = ts.Minutes.ToString
            Dim tValueHour As Integer = ts.Hours.ToString
            Dim condays As Integer = 24 * ts.Days.ToString

            totalhours = condays + tValueHour
            totalhours = totalhours / Val(frmOverdue.txthours.Text)
            frmOverdue.txtTotPay.Text = totalhours * Val(frmOverdue.txtamount.Text)
